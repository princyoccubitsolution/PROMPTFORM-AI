export function sanitizeFormTitle(rawTitle: string, rawPrompt?: string, topic?: string): string {
  ...
  // Check if raw title is heavily contaminated with prompt instructions (e.g., "Analysis This Pdf And Mcq Quiz")
  const isPromptPolluted = /\b(analysis\s+this|analysis\s+pdf|extract\s+from|created\s+a\s+quiz|generate\s+from\s+pdf)\b/i.test(cleaned);
  if (isPromptPolluted) {
    if (topic && topic.length > 2 && !/\b(analysis|pdf|document|doc|this)\b/i.test(topic)) {
      cleaned = `${topic} Quiz`;
    } else {
      cleaned = "Document Assessment Quiz";
    }
  }

  // Strip common prompt instruction prefixes and leading articles (a, an, the, modern)
  cleaned = cleaned.replace(/^(a|an|the|create|make|generate|build|please\s+create|please\s+build|please\s+generate|analysis\s+this\s+pdf\s+and|analysis\s+this|analysis\s+pdf)\s+/gi, '');
  ...
  cleaned = cleaned.replace(/\b(smart\s+field\s+types|required\s+validation|conditional\s+questions|clean\s+responsive\s+layout|booking\s+confirmation|after\s+submission|net\s+promoter\s+score|nps|rating\s+scales|anti[\s\-]?cheat|timer\s+limit|mcq|mcqs|general|structured|form\s+type|analysis\s+this\s+pdf|analysis\s+this|analysis|pdf|document)\b/gi, ' ');

  // Clean trailing and leading punctuation/whitespace
  cleaned = cleaned.replace(/^[\,\.\:\;\-\_\s]+|[\,\.\:\;\-\_\s]+$/g, '').trim();

  // Split into words
  let words = cleaned.split(/\s+/).filter(w => w.length > 0 && !["this", "pdf", "analysis", "doc", "document", "and"].includes(w.toLowerCase()));
  ...
  if (words.length === 0 || cleaned.toLowerCase() === "quiz" || cleaned.toLowerCase() === "form") {
    cleaned = topic && !/\b(analysis|pdf|doc|document)\b/i.test(topic) ? `${topic} Quiz` : "Knowledge Assessment Quiz";
  } else {
